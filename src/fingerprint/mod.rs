...
impl Fingerprint {
    ...
    pub fn encode_fp(&self, init_time_ms: i64, utc_minutes: i64) -> String {
        ...
        let part0 = vec![
            index0::platform(&self.platform, init),
            index0::vendor(&self.vendor, init),
            index0::language(&self.language, init),
            index0::device_memory(self.device_memory),
            index0::screen(
                self.screen_width,
            ...
            index0::color_depth(self.color_depth),
            index0::hardware_concurrency(self.hardware_concurrency),
            index0::device_pixel_ratio(self.device_pixel_ratio),
            index0::timezone(self.timezone_offset, self.summertime_offset),
            index0::mime_types(&self.mime_types),
            index0::plugins(&self.plugins),
            index0::navigator_bits(&self.navigator_bits),
            index0::user_agent(&self.user_agent, init),
            index0::canvas_hash(&self.canvas_hash, init),
            index0::enumerate_devices(&self.enumerate_devices_bits),
            index0::product_sub(&self.product_sub, init),
            index0::canvas_hash2(&self.canvas_hash2, init),
            index0::webgl_renderer(&self.webgl_renderer, init),
            index0::locale_date_string(&self.locale_date_string, init),
            index0::automation_bits(&self.automation_bits),
            index0::eval_length(self.eval_length),
            index0::max_call_stack_size(self.max_call_stack_size),
            index0::call_stack_error_msg(&self.call_stack_error_msg, init),
            index0::call_stack_error_name(&self.call_stack_error_name, init),
            index0::call_stack_error_stack_len(self.call_stack_error_stack_len),
            index0::touch_signature(&self.touch_signature_hex),
            index0::property_error_msg(&self.property_error_msg, init),
            index0::navigator_properties(&self.navigator_properties),
            index0::can_play_type(&self.can_play_type),
        ];
        let part4 = vec![
            index1::always0(),
            index1::time_zone(&self.time_zone, init),
            index1::languages(&self.languages, init),
            index1::vendor_number(self.vendor_number),
            index1::castle_runtime_flags(&self.castle_runtime_flags),
            index1::to_fixed_err_len(self.to_fixed_err_len),
            index1::bot_detection_flags(&self.bot_detection_flags),
            index1::ua_platform_missing(self.ua_platform_missing),
            index1::worker_integrity_flags(&self.worker_integrity_flags),
            index1::browser_feature_flags(&self.browser_feature_flags),
            index1::suspicious_environment(self.suspicious_environment),
            index1::static01(&self.static01_payload),
            index1::locale(&self.locale, init),
            index1::window_dims(
            ...
            index1::ua_high_entropy_empty(self.ua_high_entropy_empty),
            index1::ua_high_entropy_flags(&self.ua_high_entropy_flags),
            index1::ua_architecture(&self.ua_architecture, init),
            index1::ua_model(&self.ua_model, init),
            index1::ua_full_version(&self.ua_full_version, init),
        ];
        let part7 = vec![
            index2::ua_platform(&self.ua_platform, init),
            index2::ua_platform_version(&self.ua_platform_version, init),
            index2::browser_brand(&self.browser_brand),
            index2::time_diff(self.time_diff),
            index2::utc_minutes(utc_minutes),
            index2::hostname(&self.hostname, init),
            index2::object_json(&self.object_json, init),
            index2::bits46(&self.bits46),
            index2::linux_font_count(self.linux_font_count),
            index2::mac_font_count(self.mac_font_count),
            index2::windows_font_count(self.windows_font_count),
            index2::canvas_length(self.canvas_fingerprinting_len),
            index2::navigation_timing(&self.navigation_timing),
            index2::iframe_navigator_accessible(self.iframe_navigator_accessible),
            index2::canvas_integrity_flags(&self.canvas_integrity_flags),
            index2::canvas_error_message(&self.canvas_error_message, init),
            index2::memory_info(&self.memory_info),
            index2::screen_integrity_flags(&self.screen_integrity_flags),
            index2::window_dims(
            ...
            index2::avail_left_top(self.avail_left, self.avail_top),
            index2::screen_orientation(&self.screen_orientation, init),
            index2::screen_orientation_angle(self.screen_orientation_angle),
            index2::scroll_bar(self.scroll_bar_width, self.scroll_bar_height),
            index2::canvas_perf_ratio(self.canvas_perf_ratio),
            index2::voice_language(&self.voice_language, init),
            index2::voices_length(self.voices_length),
            index2::local_voices_length(self.local_voices_length),
            index2::google_voices_length(self.google_voices_length),
            index2::voice_os(&self.voice_os, init),
            index2::render_latency(self.render_latency),
            index2::keyboard_hash(&self.keyboard_hash, init),
        ];
